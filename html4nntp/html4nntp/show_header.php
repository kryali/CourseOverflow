...
<?
	function display_tree($nodes, $level, $indent = "") {
		...
		foreach ($nodes as $node) {
			...
			if ((isset($_REQUEST["article_id"])) && ($_REQUEST["article_id"]==$message_info->nntp_message_id)) {
				echo "<div id=\"image_".$message_id."\">".$voteIconHTML."</div><span class=\"msg-title\"> <strong>(+".$voteCount.") ".htmlentities(chop_str($message_info->subject, $subject_length_limit - $level*3))."</strong></span>\r\n";
			} else {
				echo "<div id=\"image_".$message_id."\">".$voteIconHTML."</div><span class=\"msg-title\"> (+<span id=\"count_".$message_id."\">".$voteCount."</span>) <a href=\"".basename($_SERVER["SCRIPT_NAME"])."?art_group=".urlencode($_SESSION["newsgroup"])."&amp;article_id=".$message_info->nntp_message_id."\">";
