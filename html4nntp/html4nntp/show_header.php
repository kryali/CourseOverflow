...
<?
	function display_tree($nodes, $level, $indent = "") {
		...
		foreach ($nodes as $node) {
			...
			if ((isset($_REQUEST["article_id"])) && ($_REQUEST["article_id"]==$message_info->nntp_message_id)) {
				echo "<span class=\"msg-title\"><strong>".htmlentities(chop_str($message_info->subject, $subject_length_limit - $level*3))."</strong></span>\r\n";
			} else {
				echo "<span class=\"msg-title\"><a href=\"".basename($_SERVER["SCRIPT_NAME"])."?art_group=".urlencode($_SESSION["newsgroup"])."&amp;article_id=".$message_info->nntp_message_id."\">";
